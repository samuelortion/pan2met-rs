...
pub fn padmet_get_misc_value(pathway_id: &String, padmet_object: &PadmetSpec, key: &String) -> Option<Vec<String>> {
    if let Some(node) = padmet_object.dic_of_nodes.get(pathway_id) {
...
pub fn padmet_pathway_key_reactions(pathway_id: &String, padmet_object: &PadmetSpec) -> Option<Vec<String>> {
...
pub fn padmet_taxonomic_range(pathway_id: &String, padmet_object: &PadmetSpec) -> Option<Vec<String>> {
    match padmet_get_misc_value(pathway_id, padmet_object, &"TAXONOMIC-RANGE".to_owned()) {
        None => None,
        Some(ranges) => {
            let taxa: Vec<String> =
                ranges.iter().filter(
                    |taxon| taxon.starts_with("TAX-")
                ).map(
                    |taxon| taxon.replace("TAX-", "")
                ).collect();
            Some(taxa)
        }
    }
